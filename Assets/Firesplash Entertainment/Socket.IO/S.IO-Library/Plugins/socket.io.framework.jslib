mergeInto(LibraryManager.library, {
	...
	CreateSIOInstance: function (instanceName, targetAddress) {
		...
		window.UnitySocketIOInstances[iName].on('connect', function() {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 1); //connected
		});
		
		window.UnitySocketIOInstances[iName].on('disconnect', function(reason) {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 0); //disconnected
		});
		...
		window.UnitySocketIOInstances[iName].on('connect_timeout', function() {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 2); //errored
			unityInstance.SendMessage(iName, 'SIOWarningRelay', 'Timeout on connection ' + iName);
		});
		
		window.UnitySocketIOInstances[iName].on('connect_error', function(error) {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 2); //errored
			unityInstance.SendMessage(iName, 'SIOWarningRelay', 'Error on connection attempt for ' + iName + ': ' + error);
		});
		...
		window.UnitySocketIOInstances[iName].on('reconnect_attempt', function() {
			...
			unityInstance.SendMessage(iName, 'SIOWarningRelay', 'Websocket failed for ' + iName + '. Trying to reconnect with polling enabled.');
		});
		
		window.UnitySocketIOInstances[iName].on('reconnect_error', function(error) {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 2); //errored
			unityInstance.SendMessage(iName, 'SIOWarningRelay', 'Error on reconnection attempt for ' + iName + ': ' + error);
		});
		
		window.UnitySocketIOInstances[iName].on('reconnect_failed', function(error) {
			unityInstance.SendMessage(iName, 'UpdateSIOStatus', 2); //errored
			unityInstance.SendMessage(iName, 'SIOWarningRelay', 'Reconnect failed for ' + iName + ': Max. attempts exceeded.');
		});
	},
	...
	RegisterSIOEvent: function (instanceName, eventName) {
		...
		window.UnitySocketIOInstances[iName].on(eName, function (data) {
			unityInstance.SendMessage(iName, 'RaiseSIOEvent', JSON.stringify({
				eventName: eName,
